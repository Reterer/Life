...
class Environment:
	...
	def generate_bot(self, i):
		for i in range(i):
			...
			while self.world[x][y][0] != 0:
				x, y = random.randint(0, config.WIDTH_MAP - 1), random.randint(0, config.HEIGHT_MAP - 1)
			self.world[x][y] = [0, i, config.STANDARD_RADIUS_BOT]
	...
	def generate_food(self, i):
		for i in range(i):
			...
			yield Food(x, y, 7, [0, 255, 0], 200)

	def _setup(self, world=None, bots=None, food=None, epoch=0, iter_for_epoch=config.ITER_FOR_EPOCH, crt_iter=0):
		self.epoch = epoch
		...
		if world:
			...
			self.world = [[[0, None, None] for i in range(config.HEIGHT_MAP)] for j in
						  range(config.WIDTH_MAP)]  # Карта мира [x,y], все вещи имеют int координаты
			for i in range(config.WIDTH_MAP):
				self.world[i][0] = [3, None, None]
				self.world[i][-1] = [3, None, None]
			for i in range(config.HEIGHT_MAP):
	...
	def _generate_new_food(self, id_food):
		...
		x = random.randint(0, config.WIDTH_MAP - 1)
		y = random.randint(0, config.HEIGHT_MAP - 1)
		while self.world[x][y][0] != 0:
			x = random.randint(0, config.WIDTH_MAP - 1)
			y = random.randint(0, config.HEIGHT_MAP - 1)
	...
	def _collision(self, i_bot):
		...
		for x in range(max(x_bot - 2 * r_bot, 0), min(x_bot + 2 * r_bot, config.WIDTH_MAP)):
			for y in range(max(y_bot - 2 * r_bot, 0), min(y_bot + 2 * r_bot, config.HEIGHT_MAP)):
	...
	def save(self):
		if len(self.bots) > 0 and self.last_save + config.AUTO_SAVE_INTERVAL < time.time():
			self.last_save = time.time()
	...
	def new_epoch(self):
		...
		if len_bots > 0:
			...
			for i in range(len(new_bots)):
				x_bot, y_bot = random.randint(0, config.WIDTH_MAP - 1), random.randint(0, config.HEIGHT_MAP - 1)
	...
	def update(self):
		...
		while i < len(self.bots):
			...
			if duplicate == 1:
				...
				new_vel[0] += dx - new_vel[0] * config.F_FRICTION * config.DELTA_TIME
				new_vel[1] += dy - new_vel[1] * config.F_FRICTION * config.DELTA_TIME

				x = self.bots[i].x + new_vel[0] * config.DELTA_TIME
				y = self.bots[i].y + new_vel[1] * config.DELTA_TIME

				if x >= config.WIDTH_MAP:
					x = config.WIDTH_MAP - 1
					new_vel[0] = 0
				if x < 0:
					x = 0
					new_vel[0] = 0
				if y >= config.HEIGHT_MAP:
					y = config.HEIGHT_MAP - 1
					new_vel[1] = 0
				if y < 0:
					y = 0
					new_vel[1] = 0
				...
				self.bots[i].radius = config.STANDARD_RADIUS_BOT + int(self.bots[i].energy // 100)
